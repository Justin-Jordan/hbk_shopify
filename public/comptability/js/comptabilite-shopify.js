jQuery(document).ready(function($) {
	...
	Vue.component('createorder', {
		...
		mounted: function() {
			...
			// console.log(this.modedebug);
			// this.modedebug = false;
			/**
			 * add commande number
			 */
			// $('#model-create-invoice .modal-header .group-logos
			// .commande').html('0000')
		...
		methods: {
			...
			get_token: function(event) {
				...
				if (DatasReturn && DatasReturn.results) {
					...
					success: function(data) {
						...
						console.log(data); {
							...
							if (data.salereceipt_result) {
								...
								self.successAjax(' <strong>Facture annulé</strong> ',
									displayMassage);
								self.accordions.push({
									'title': 'Facture, flux',
			...
			errorAjax: function(error = '', classe = 'alert-danger') {
				...
				var message =
					'<strong class="d-block mb-3"> Echec de la sauvegarde  ... </strong>';
				if (error.statusText) {
			...
			traitementErrorQB: function(error) {
				...
				if (n) {
					...
					if (texte && texte['Fault'] && texte['Fault']['Error']) {
						...
						if (texte['Fault']['Error']['actions'] && texte['Fault']['Error'][
								'actions'
							]['next_code_facture']) {
							messageError +=
								'<span class="d-block"> <button class="btn btn-outline-primary btn-sm mt-3" onclick="next_code_facture()">Réessayer avec le numéro suivant</button> </span>';
						}
						return messageError;
					} else {
	...
	var commandes = new Vue({
		...
		methods: {
			...
			saveSelectDate: function() {
				...
				let input_firstday = getDateinFrench(this.input_firstday, separateur =
					'-');
				let input_lastday = getDateinFrench(this.input_lastday, separateur =
					'-');
		...
		components: {
			'liste_badge': {
				...
				computed: {
					...
					card_info_infos: {
						get: function() {
							return (this.current_stat.card_info_infos) ? add_EURO(this.current_stat
								.card_info_infos) : '0.00€';
						}
					},
					card_info_color: {
						get: function() {
	...
	function buildArray(rows, filedsPlus) {
		...
		if (displayTotal) {
			...
			$.each(valide_key, function(index, value) {
				if (value.total) {
					results['total'][value.name] = '';
					if (value.total.action == 'SUM') {
						results['total'][value.name] = getSUM(value.name, results);
					}
				} else if (ref == value.name) {
					results['total'][value.name] = 'Total';
				} else {
					results['total'][value.name] = null;
				}
	...
	function getSUM(name, results) {
		var sum = 0;
		$.each(results, function(index, value) {
			if (value[name] && index != "total") {
				sum = sum + parseFloat(value[name]);
			}
		});
	...
	// /// fonction permettant d'effectuer des arrondis
	function getDateinFrench(data, separateur = '/') {
		var _date = new Date(data);
		var day = _date.getDate().toString().padStart(2, '0');
		var mounth = (_date.getMonth() + 1).toString().padStart(2, '0');
		var year = _date.getFullYear();
		if (separateur == '/') {
			var french_date = day + '/' + mounth + '/' + year;
		} else if (separateur == '-') {
			var french_date = day + '-' + mounth + '-' + year;
		}
		return french_date;
