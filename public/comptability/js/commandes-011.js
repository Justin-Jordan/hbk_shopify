jQuery(document).ready(function($) {
	...
	Vue.component('button_next_code', {
		...
		data: function() {
			return {
				text_button: 'RÃ©essayer avec le numÃ©ro suivant',
			}
		},
		mounted: function() {
			if (this.sub_datas && this.sub_datas.use_next_code_DocNumber) {
				this.text_button = 'RÃ©essayer avec le numÃ©ro suivant  (+' + this.sub_datas
	...
	Vue.component('createorder', {
		...
		mounted: function() {
			 ...
			 * Get mode
			 */
			this.modedebug = parseInt(Cookies.get('createorder_modedebug'));
		...
		methods: {
			...
			create_invoice_andpayement_test: function(event) {
				this.resetFields();
				var datas = this.modal_body;
				this.execute_action(event, datas, action =
					'create_invoice_andpayement_test');
			},
			...
			create_invoice_all_next_code_facture: function() {
				...
				this.execute_action(event, datas, action =
					'create_invoice_all_next_code_facture');
			},
			...
			get_token: function(event) {
				var datas = {};
				var DatasReturn = this.execute_action(event, datas, action =
					'get_token', false, asyn = true, displayMassage = false);
			...
			getInvoice: function() {
				...
				this.execute_action(event, datas, action = 'get_invoice', asyn = true,
			...
			getCart: function() {
				...
				this.execute_action(event, datas, action = 'get_cart', asyn = true,
					displayMassage = false);
				//
			},
			...
			void_invoice: function() {
				...
				if (self.all_datas_form_cart.cart) {
					...
					this.modal_body['dafault']['status_invoice'] = self.all_datas_form_cart
						.status_invoice;
				}
				this.resetFields();
			...
			execute_action: function(event, datas, action = 'get_invoice', asyn =
				true, displayMassage = true) {
				var self = this;
						...
						console.log(data); {
							...
							if (data.salereceipt_result) {
								...
								if (data.salereceipt_result.Fault) {
									...
									self.successAjax(' <strong>RÃ©Ã§u crÃ©e</strong> ');
								}
								...
								if (data.invoice_result && data.invoice_result.Invoice) {
									...
									if (self.status_invoice == 'SUCCESS' || self.status_invoice ==
										'TEST') {
										...
										message += (
											' <strong class="d-block">Facture crÃ©e , Reference : ' +
											data.invoice_result.Invoice.DocNumber + '</strong> ');

										self.accordions.push({
											'title': 'Facture, flux envoyÃ©',
											body: data.invoice_send
										});
										self.accordions.push({
											'title': 'Facture, flux retournÃ©',
										...
										message += (
											' <strong class="d-block">La facture existe dÃ©jÃ , Reference : ' +
											data.invoice_result.Invoice.DocNumber + '</strong> ');
										...
										classe = 'alert-warning';
										message += (
											' <strong class="d-block "> La facture n\'a pas Ã©tÃ© crÃ©e </strong> '
										);
									}
								}
								if (data.payments_result && data.payments_result.Payment) {
									...
									if (self.status_payment == 'SUCCESS' || self.status_payment ==
										'TEST') {
										...
										message +=
											' <strong class="d-block">Paiement crÃ©e, Reference : ' +
											data.payments_result.Payment.PaymentRefNum + '</strong> ';

										if (!check_invoice) {
											self.accordions.push({
												title: 'Payement, flux envoyÃ©',
												body: data.payment_send
											});
											self.accordions.push({
												title: 'Payement, flux retournÃ©',
										...
										classe = 'alert-warning';
										message +=
											' <strong class="d-block">Le paiement existe dÃ©jÃ , Reference : ' +
											data.payments_result.Payment.PaymentRefNum + '</strong> ';
										if (!check_invoice) {
											self.accordions.push({
												title: 'Payement, flux envoyÃ©',
										...
										classe = 'alert-warning';
										message += (
											' <strong class="d-block"> Le paiement n\'a pas Ã©tÃ© crÃ©e </strong> '
										);
									}
								}
								...
								if (
									(data.cart_order && data.cart_order.admin_graphql_api_id &&
								...
								if (data.buildCart.account) {
									 ...
									 */
									if (data.buildCart.account.TransactionLocationType ==
										'WithinFrance') {
										data.buildCart.account.TransactionLocationType = 'France';
										self.account = data.buildCart.account;
									} else if (data.buildCart.account.TransactionLocationType ==
										'FranceOverseas') {
								...
								self.successAjax(
									' <strong>JSON du payment generÃ©, creation dÃ©sactiver</strong> ',
									displayMassage);
							} else if (data.VoidInvoice) {
								self.successAjax(' <strong>Facture annulÃ©</strong> ',
									displayMassage);
								self.accordions.push({
									'title': 'Facture, flux',
								...
								self.successAjax(
									' <strong>DonnÃ©es sauvergardÃ©s avec succes</strong> ',
									displayMassage);
							}
						}
			...
			alert_action: function(datas) {
				...
				if (datas.status) {
					...
					if (datas.action && datas.action === 'create_customer') {
						...
						if (parseInt(this.modal_body.modedebug) < 0 || parseInt(this.modedebug) < 0) {
							event = {
								target: $('#model-create-invoice .modal-header'),
							};
			...
			successAjax: function(message = '', display = true, classe =
				'alert-success') {
				if (!display) {
					return false;
				}
			...
			errorAjax: function(error = '', classe = 'alert-danger') {
				...
				if (error.status == 405) {
					...
					message =
						'<strong class="d-block mb-3">Delai d\'attente dÃ©passÃ©, verifier votre connexion ou rÃ©Ã©ssayer plus tard.</strong>';
					self.alert_message_static = message;
					return true;
				}
				/**
				...
				if ((self.modedebug < 0)) {
					self.alert_message_static =
						"<strong> Une erreur s'est produite, rÃ©essayer plus tard, ou contactez nous si cela persiste </strong>";
					return false;
				}
				var message = '';
				if (error.statusText) {
					message = '<strong class="d-block mb-3"> ' + decodeURIComponent(error
						.statusText) + '</strong>';
				}
				if (errorQB) {
					self.alert_attribut_class_static = 'alert-warning';
			...
			traitementErrorQB: function(error) {
				...
				if (n) {
					var messageError = '<strong class="d-block mb-3">' +
						decodeURIComponent(error.statusText) + '</strong>';
					...
					if (texte && texte['Fault'] && texte['Fault']['Error']) {
						$.each(texte['Fault']['Error'], function(i, k) {
							if (k.Message && k.Message != '') {
								messageError += '<span class="d-block"> - ' + k.Message +
									'</span>';
							}
							if (k.Detail && k.Detail != '') {
								messageError += '<span class="d-block"> - ' + k.Detail +
									'</span>';
							}
							if (k.Detail) {
								// messageError +='<span
			...
			open_windows: function(event) {
				...
				window.open(this.token_url_refresh, "window", "width= " + w +
						...
						if (datas.results.data) {
							$.each(datas.results.data, function(i, v) {
								if (v.type && v.type == 'create_customer') {
									...
									if (v.is_new) {
										message =
											'<strong class="d-block">Client crÃ©e : ' + v.value.DisplayName +
											'</strong>';
									} else {
										message =
											'<strong class="d-block">Le client existe deja : ' + v.value.DisplayName +
											'</strong>';
									}
	...
	var commandes = new Vue({
		...
		data: {
			...
			// tbleau de status
			/*
			 * headers_status:{ 'status':{label:'Ã‰tat'},
			 ...
			 * bases de donnÃ©es, BD name, user ...
			 */
			...
			/**
			 * Contient les elments Ã  filtrer via SQL.
			 */
			filters: {},
		...
		methods: {
			...
			active_modal_create_invoice: function(index) {
				var self = this;
				// console.log('index from ev_create_invoice: ', index);
				if (self.modedebug > 0) {
					index['modedebug'] = 1;
				} else {
					index['modedebug'] = -1;
				...
				self.create_invoice_modal_body.modedebug = self.config__manage_connect
					.modedebug;
	...
	function getDateinFrenchByTimestamp(data) {
		if (data) {
			...
			// french_date += ' Ã  '+_date.getUTCHours() +'H'+ mn;
			return french_date;
		}
	}
	...
	function getMysqlDateToFrench(data) {
		if (data && data != '') {
			...
			french_date = (_date.getDate()).toString().padStart(2, '0') + '/' + (_date
				...
				' Ã  ' + (_date.getHours()).toString().padStart(2, '0') + ':' + (_date.getMinutes())
	...
	/**
	 * Enregistre une donnÃ©e en session.
	 */
	...
	function paiement_status(key) {
		var status = {
			// 'pending':'En attente',
			// 'partially_shipped':'Partiellement payÃ©',
		};
		if (status[key]) {
			return status[key];
		}
