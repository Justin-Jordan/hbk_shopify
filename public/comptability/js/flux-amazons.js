  ...
  var fluxAmazon = new Vue({
    ...
    methods:{
        ...
        $.ajax({
          ...
          success : function(data) { console.log(data);
           ...
           //console.log(data.donnee_fiscale_shopify[0]);
            ...
            self.product_ht_ca_shopify = add_EURO(parseFloat(data.donnee_fiscale_shopify[0].panier_ht) + parseFloat(data.donnee_fiscale_shopify[0].port_ht));//add_EURO( data.donnee_fiscale_shopify[0].total_paid );
            ...
            self.product_ht_ca_tva_amazon = add_EURO(data.tva['somme_rq1_rq2']);  
            
            // frais de port = frais de port commande - frais de port remboursé
            self.frais_port_amazon = '<span class="d-block">';
            self.frais_port_amazon += (data.shipping_amazon.shipping) ? add_EURO(data.shipping_amazon.shipping) : '0 €';
            ...
            self.frais_port_tva_amazon = add_EURO(data.ShippingTax_amazon.ShippingTax);
      ...
      buildDataChart: function(data){
        ...
        }else if(self.input_firstday == '' && self.input_lastday !=''){console.log('self.input_lastday ')
          max_label=100;
          var today = new Date();
          ...
          $.each(value.perfom, function(i, f){
            if(f=='getDateinFrench'){
              ...
              if('panier_ttc' == value.name){
                var monney = precisionRound(results[id][value.name]);// console.log(results[id][value.name]+' : '+total.monney);
                total.monney +=monney;
              }
